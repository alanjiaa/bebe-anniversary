// pages/checkout.js
'use client'               // mark as client component in App Router; omit if using Pages Router
import { useState, useEffect } from 'react'
import Link from 'next/link'
import { jsPDF } from 'jspdf'
import 'jspdf/dist/polyfills.es.js'  // ensure jsPDF works in Next
...
export default function ReceiptPage() {
  ...
  const downloadPDF = () => {
    ...
    let y = 120
    doc.setDrawColor('#FADADD') // background stripe
    doc.setLineWidth(10)
    ...
    doc.setFont('helvetica', 'bold')
    doc.text(
      `Total Spent: ${purchase.total} pts`,
      40,
      y += 30
    )
    ...
    doc.text(
      `Thank you for shopping at Alsie Shop! 💖`,
      40,
      750
    )
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-cream p-8">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-lg w-full max-w-md">
        <p className="mb-2"><strong>Date:</strong> {new Date(purchase.date).toLocaleString()}</p>
      ...
      <Link
        href="/home"
        className="mt-6 text-soft-lavender underline hover:text-soft-lavender/90"
      >
        ← Back to Menu
      </Link>
    </div>
